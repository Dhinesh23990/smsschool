...
var transformResponse = function(results){	
	...
	var $injector = angular.element(e).injector();
	var $rootScope = $injector.get('$rootScope');	
	/**Vulnerability checking - redirect to login*/
	if(results.status === 403 ){		
		//_removeUserData('FORBIDDEN');
	...
	if(response && response.toLowerCase().indexOf('session expired') >= 0  && results.config.url.indexOf('.html') >= 0){
		$rootScope.logout('LOGGED_OUT');
	}
...
function getChartSeriesFromObj(chartFields,chartData,type){
	var chartSeries = [];
