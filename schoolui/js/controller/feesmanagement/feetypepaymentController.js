...
//					});
		...
		method:'GET'
			...
			$scope.getFeeConfigTerm = function(batchId,studentId,term,studentName){
				...
				$http ({
					...
					$scope.getAllPaymentModeBySchoolId();

					if($scope.feesConfigDetails.status == null && feesConfigDetails.status === 'paid'){
						$state.go('feespaymenttype', {}, {reload: true});
					}
	...
	/*		$scope.paymentsuccess = function(){
			...
			$scope.getAllPaymentModeBySchoolId =  function() {	   
				$http({
					url:appCon.globalCon.serviceBaseURL+"paymentMode/getAllPaymentModeBySchoolId?shoolId="+$cookieStore.get('schoolId'),
					method:'GET'
				}).success(function(result){
					if(result.statusFlag === 'Ok'){
			...
			$scope.feesPayUpdate = function(students,feesConfig,feePay) {
				
				console.log(feesConfig)
