(function() {
     ...
     */
    angular.module(appCon.appName).directive('appDataTable', function() {
    	return {
            restrict: 'E',
            scope: true,
            controller: 'appDataTableController'
        };
    });

    angular.module(appCon.appName).controller('appDataTableController', ['$scope', '$filter', 'ngTableParams', '$injector', '$element', '$attrs', '$parse', '$rootScope', '$interpolate','$window','$timeout',  
        function($scope, $filter, ngTableParams, $injector, $element, $attrs, $parse, $rootScope, $interpolate, $window, $timeout) {
            ...
            if (initialRequest !== undefined) {
                ...
                config = JSON.parse(initialRequest.replace(/'/g, '"'));
            }
	                            ...
	                            function(result) {
	                            	if (result.data.status === 'success') {
	                            		...
	                            		if(angular.isDefined($scope.checkAll)){						
    										var getTrueValue = 0,
    										    getCurrentRecords = $parse(rootNode)($scope);
    										angular.forEach(getCurrentRecords, function(item) {						
    										...
    										if(getTrueValue && getCurrentRecords.length === getTrueValue){
			...
			//handle parent check
            $scope.handleParentCheckbox = function(){
				var getTrueValue = 0;
     ...
     * <div class="clearfix" 
     *    app-data-table-export="employeeList" 
     *    service="organizationServices" 
     *    operation="searchEmployeeWithFilters"
     *    params='{"param1":"value1","param2":"value2"}'
     *    search-type="searchRequest" 
     ...
     *  </div>
     */
    angular.module(appCon.appName).directive('appDataTableExport', ['$injector','$http','$interpolate','$parse',
        function($injector, $http, $interpolate, $parse) {
            return {
                restrict: 'A',
                scope: {
                    params: '=?appDataTableExport',
                    exportType: '@'
                },
                template: '<span class="dataTableExport pull-left">' +
