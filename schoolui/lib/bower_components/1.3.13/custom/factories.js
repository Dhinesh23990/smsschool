        ...
        request: function(config) {
        	if(config.url.indexOf('.json') >= 0  && config.url.indexOf('messages_properties_') >= 0){
        		...
        		if(appCon.cookie.getItem('loginusertoken')){
        			var loginusertoken = appCon.cookie.getItem('loginusertoken');
        			config.url = config.url+'?authType=token&authorization='+loginusertoken;
        		}else{
        			config.url = config.url + '?' + appCon.globalCon.deployDate;
        		}
        		...
        		if(config.url.indexOf('?')>=0){
        			if(appCon.cookie.getItem('loginusertoken')){
            			config.url = config.url+'&authType=token&authorization='+appCon.cookie.getItem('loginusertoken');
            		}else{
