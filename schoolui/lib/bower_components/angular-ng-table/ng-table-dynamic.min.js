					    	...
					    	switch(numberOption){
					    		case 'btw':
					    				if(!hasErrors && (!testNumber.test(numberStart) || numberStart === '')){
					    					...
					    					$scope.params.parameters().filter[filterName] = $scope.params.parameters().filter[filterName+'_start'] +'#btw#'+ $scope.params.parameters().filter[filterName+'_end'];
					    				}
					    			break;
					    		default:
					    			if(!hasErrors && (!testNumber.test(numberEnd) || numberEnd === '')){
										...
										if(numberOption === 'eq'){
											$scope.params.parameters().filter[filterName] = $scope.params.parameters().filter[filterName+'_end'];
										}else if (numberOption === ' '){
											$scope.params.parameters().filter[filterName] = '';
										}else{
											$scope.params.parameters().filter[filterName] = '#' +numberOption+ '#'+ $scope.params.parameters().filter[filterName+'_end'];
										}
									}
					    			break;    	
					    	}
...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
	...
	$templateCache.put('ng-table/filters/text.html', '<input type="text" id="{{name}}" placeholder="{{$column.placeHolder(this) || $column.title(this) || $column.titleAlt(this)}}" ng-keyup="doTextSubmit($event)" maxlength="{{::$column.maxLength()}}" name="{{::name}}" ng-disabled="$filterRow.disabled" ng-model-options="{ debounce: 50 }"  ng-model="params.filter()[name]" ng-if="filter==\'text\'" class="input-filter form-control"/>');
	...
	$templateCache.put('ng-table/filters/number.html','<div class="form-inline" ng-init="optionsValName = name+\'_options\'; startValName = name+\'_start\'; endValName = name+\'_end\'; showStartVal=(params.filter()[optionsValName].id == \'btw\') ? true:false;; ">'+
		   '<input id="{{name}}_Start" class="input-filter form-control pull-left" placeholder="{{$column.placeHolder()}}" style="background-color:#fff;width:50px;float:left;" ng-keyup="doNumberFilter($event, optionsValName, startValName, endValName)" type="text" ng-model="params.filter()[startValName]" ng-show="showStartVal" />'+
		   ...
		   '<input id="{{name}}_End" class="input-filter form-control pull-left" placeholder="{{$column.placeHolder()}}" ng-readonly="params.filter()[optionsValName].id == \'\' || params.filter()[optionsValName].id == \' \' || !params.filter()[optionsValName].id" style="width:50px;float:left;" type="text" ng-model="params.filter()[endValName]"  ng-keyup="doNumberFilter($event, optionsValName, startValName, endValName)"/>'+
